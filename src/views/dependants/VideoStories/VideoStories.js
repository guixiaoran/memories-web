...
const VideoStory = (props) => {
  ...
  useEffect(() => {
    if (player === undefined)
      setPlayer(
        new Plyr(document.getElementById(props.video._id), {
          playsinline: true,
          clickToPlay: false,
  ...
  useEffect(() => {
    if (player !== undefined) if (props.isVisible === false) player.pause();
  ...
  return (
    <div style={{ width: "100%" }}>
      <AnimatedObject initial="right">
        <ParallaxHelper blur={1} bgImage={parallaxBG} strength={900}>
          <div style={{ width: "100vw" }}>
            ...
            >
              <InputLabel style={{ color: "white " }} id="select-label">
                Type
              </InputLabel>
              <Select
              ...
              >
                ...
                <MenuItem value={"STORYTELLER"}>
                  Cantastorie / Story Tellers
                </MenuItem>
              </Select>
            </FormControl>
            {videoStories
              .filter((story) => story.type.toUpperCase() === select)
              .map((video, index) => {
                return (
                  <div
                    key={`videoStory${index}`}
                    style={
                      index === videoStories.length - 1 && !desktop
                        ? {
                            paddingBottom: "20vh",
                          }
                        : {
